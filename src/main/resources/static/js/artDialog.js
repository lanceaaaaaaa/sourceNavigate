...
$.extend(Popup.prototype, {
    ...
    __center: function (winPosition) {
        ...
        if(this.winPosition){
            if(this.winPosition[0].right)  style.right = this.winPosition[0].right + 'px'; style.left = 'inherit';
            this.winPosition[0].left ? style.left = this.winPosition[0].left + 'px' : null;
            this.winPosition[1].top ? style.top = this.winPosition[1].top + 'px' : null;
            this.winPosition[1].bottom ? style.bottom = this.winPosition[1].bottom + 'px' : null;
        }else{
...
function post(name,path) {
    ...
    form.attr("action", path.split("?")[0]);
    form.attr("target",name);
    parameters = getUrlArgObject(path);
...
dialog.oncreate = function (api) {
    ...
    // 页面加载URL的请求方式(默认为空是POST，可以防止参数乱码，GET可以防止dialog reload时浏览器弹出询问框)
